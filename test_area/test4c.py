...
def convert_black_to_white(batchX, crop_factor=0.9, noise_factor=0.6, crop=0, noise=1):
    batchX = np.where(batchX <= 30, 255, batchX)   #batchX = np.where(batchX <= 30, 255, batchX)
    y, x, _ = batchX.shape
...
test2_it = datagen_real.flow_from_directory(path_test2, class_mode=class_mode, target_size=target_size, color_mode=color_mode, batch_size=batch_size, seed=seed)

# confirm the iterator works
batchX, batchy = train_it.next()
print('Batch shape=%s, min=%.3f, max=%.3f' % (batchX.shape, batchX.min(), batchX.max()))
#plt.imshow(batchX[15,:,:,:].reshape(dim_xy,dim_xy), cmap='binary')

#test2_X, test2y = test2_it.next()
#plt.imshow(test2_X[3,:,:,:].reshape(dim_xy,dim_xy), cmap='binary')

def create_model():
    model = Sequential()

    model.add(Conv2D(64, kernel_size=5, activation='relu', padding='same', input_shape=(dim_xy, dim_xy, 1)))
